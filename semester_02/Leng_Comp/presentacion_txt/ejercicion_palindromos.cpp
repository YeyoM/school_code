...
void evaluar(string texto) {
  ...
  reverse = texto;
    ...
    for(int i = 0; i < texto.length(); i++) {
      // evalua si cada letra es igual, la variable texto que 
      // es la que leyo del archivo y la variable reverse que 
      // es la copia de texto pero invertida
      if(texto[i] == reverse[i]) {
        igual++;
      }
    }
    // si el contador de las veces que fue igual es 
    // indentico al tama�o de letras de la palabra 
    // entonces si es palindroma

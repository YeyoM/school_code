...
void sacarNodo(Nodo *&pila, int &cima, char *expresion_prefijo, int &cuenta_prefijo);
void vaciarPilaParentesisAbierto(Nodo *&pila, int &cima, char *expresion_prefijo, int &cuenta_prefijo);
void vaciarPila(Nodo *&pila, int &cima, char *expresion_prefijo, int &cuenta_prefijo);
...
int main() {
  ...
  for (int i = 0; i < strlen(expresion) / 2; i++) {
    char temp = expresion[i];
    expresion[i] = expresion[strlen(expresion) - 1 - i];
    expresion[strlen(expresion) - 1 - i] = temp;
  }
  ...
  for (int i = 0; i < longitud_expresion; i++) {
    ...
    if (expresion[i] == '(') {
      ...
      if (expresion[i] == ')') {
        ...
        vaciarPilaParentesisAbierto(pila_expresion, cima_expresion, expresion_prefijo, cuenta_prefijo);
        // este ultimo es para sacar al parentesis que abre
        sacarNodo(pila_expresion, cima_expresion, expresion_prefijo, cuenta_prefijo);
      } else {
        if ( // verificamos si el caracter es un operando
          ...
          expresion_prefijo[cuenta_prefijo] = expresion[i];
          cuenta_prefijo++;
        } else {
  ...
  if(cima_expresion != -1) {
    vaciarPila(pila_expresion, cima_expresion, expresion_prefijo, cuenta_prefijo);
  }

  for (int i = 0; i < strlen(expresion_prefijo) / 2; i++) {
    char temp = expresion_prefijo[i];
    expresion_prefijo[i] = expresion_prefijo[strlen(expresion_prefijo) - 1 - i];
    expresion_prefijo[strlen(expresion_prefijo) - 1 - i] = temp;
  }
  cout << "Expresion en prefijo: " << expresion_prefijo << endl;
...
void sacarNodo(Nodo *&pila, int &cima, char *expresion_prefijo, int &cuenta_prefijo) {
  ...
  if(pila -> caracter != '(' || pila -> caracter != ')') {
    expresion_prefijo[cuenta_prefijo] = pila -> caracter;
    cuenta_prefijo++;
  }
  pila = pila -> siguiente;
...
void vaciarPilaParentesisAbierto(Nodo *&pila, int &cima, char *expresion_prefijo, int &cuenta_prefijo) {
  ...
  while(nodo_iterador -> caracter != '(') {
    if(nodo_iterador -> caracter != '(' || nodo_iterador -> caracter != ')') {
      expresion_prefijo[cuenta_prefijo] = nodo_iterador -> caracter;
      cuenta_prefijo++;
    }

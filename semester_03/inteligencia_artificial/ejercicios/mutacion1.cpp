...
int main() {
  ...
  for (int k = 0; k < NUMERO_DE_CADENAS / 2; k++) {
    ...
    */
    ...
    if (k != 0) {
      ...
      // Seleccionamos 2 elementos al azar de la generacion de los ultimos 5
      int indicesAleatorios[2];
      int seleccionados = 0;
      int iterador = 0;

      while (seleccionados < 2) {
        int indiceAleatorio = aleatorioEnRango(5, 9);
        bool exists = std::find(std::begin(indicesAleatorios), std::end(indicesAleatorios), indiceAleatorio) != std::end(indicesAleatorios);
        if (!exists) {
          indicesAleatorios[iterador] = indiceAleatorio;
          iterador++;
      ...
      // Mutar los dos elementos seleccionados
      for (int i = 0; i < 2; i++) {
        // elegir un porcentaje del 10% al 40% (1 a 4)
        int porcentaje = 1 + rand() % (4 - 1); 
        // elegir n indices al azar
        int indices[porcentaje];
        int seleccionados = 0;
        int iterador = 0;
        while (seleccionados < porcentaje) {
          int indiceAleatorio = aleatorioEnRango(0, 9);
          bool exists = std::find(std::begin(indices), std::end(indices), indiceAleatorio) != std::end(indices);
          if (!exists) {
            indices[iterador] = indiceAleatorio;
            iterador++;
            seleccionados++;
          }
        }

        // Mutar los indices seleccionados
        for (int j = 0; j < porcentaje; j++) {
          if (binarios1[k][indicesAleatorios[i]][indices[j]] == '0') {
            binarios1[k][indicesAleatorios[i]][indices[j]] = '1';
          } else {
            binarios1[k][indicesAleatorios[i]][indices[j]] = '0';
          }
        }
      }
  ...
  for (int k = 0; k < NUMERO_DE_CADENAS / 2; k++) {
    ...
    if (k != 0) {
      ...
      for (int i = 0; i < 2; i++) {
        // elegir un porcentaje del 10% al 40% (1 a 4)
        int porcentaje = 1 + rand() % (4 - 1); //hola
        //hola yeyo
        //yeyo hola
        // elegir n indices al azar
        int indices[porcentaje];
